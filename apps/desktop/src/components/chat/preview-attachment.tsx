...
export function PreviewAttachment({ source = 'manual', target }: { source?: PreviewRecordSource; target: string }) {
  ...
  return (
    <div className="flex w-full max-w-160 flex-wrap items-center gap-2.5 rounded-lg border border-border bg-card px-2.5 py-1.5 text-sm">
      <span className="grid size-7 shrink-0 place-items-center rounded-md bg-muted text-muted-foreground">
        <MonitorPlay className="size-3.5" />
      </span>
      <div className="min-w-0 flex-1">
        <div className="truncate text-[0.78rem] font-medium leading-[1.15rem] text-(--ui-text-secondary)">{name}</div>
        <div className="truncate font-mono text-[0.66rem] leading-4 text-muted-foreground">{target}</div>
      </div>
      <button
        className="ml-auto shrink-0 rounded-md border border-border bg-background px-2 py-1 text-[0.7rem] font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground disabled:opacity-50 max-[28rem]:ml-9 max-[28rem]:w-[calc(100%-2.25rem)]"
        disabled={opening}
