...
describe('useRouteResume', () => {
  ...
  it('resumes when pathname changes to a routed session', () => {
    ...
    expect(resumeSession).toHaveBeenCalledWith('session-2', true, false)
  })

  it('forces a backend resume when the gateway reconnects to an already-active route', () => {
    const resumeSession = vi.fn(async () => undefined)
    const startFreshSessionDraft = vi.fn()
    const activeSessionIdRef: MutableRefObject<null | string> = { current: 'runtime-1' }
    const creatingSessionRef = { current: false }
    ...
    expect(resumeSession).toHaveBeenCalledWith('session-1', true, true)
  })
})
