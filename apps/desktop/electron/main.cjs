...
function readDesktopConnectionConfig() {
  ...
  let config = { mode: defaultMode, remote: {}, profiles: {} }
...
async function spawnPoolBackend(profile, entry) {
  ...
  // Remote-only fork: never spawn local per-profile backends. Remote profile
  // overrides and global remote profiles are handled above.
  if (REMOTE_ONLY) {
